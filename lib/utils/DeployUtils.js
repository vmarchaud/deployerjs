    ...
    async.series([
      ...
      function (next) {
        ...
        server.conn.sftp((err, sftp) => {
          ...
          server.reporter.update(server.name, 'Making a backup of remote source folder');
      ...
      function (next) {
        ...
        server.conn.sftp((err, sftp) => {
          ...
          server.reporter.update(server.name, 'Making a backup of remote current folder');
      ...
      function (next) {
        ...
        server.conn.sftp((err, sftp) => {
          ...
          server.reporter.update(server.name, 'Creating source folder');
          sftp.mkdir(server.path + '/source', next)

          // source folder is now created
          opts.source = FileState.VALID;
        })
      },
      ...
      function (next) {
        ...
        server.conn.sftp((err, sftp) => {
          ...
          server.reporter.update(server.name, 'Create current symlink to current');
          sftp.symlink(server.path + '/source', server.path + '/current', next);
        })
      },
      // now we have a valid remote setup, so if the source folder 
      function (next) {
        if (opts.source !== FileState.VALID) return next(null);

        return server.strategy.retrieve(next);
      }
    ], function (err) {
      return cb(err);
    })
