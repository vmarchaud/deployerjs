...
module.exports = class Deployer {
  ...
  */
  select (target, opts, cb) {
    ...
    async.eachLimit(Object.keys(servers), 1, (server_name, next) => {
      ...
      if ((typeof (server.password) === 'boolean' && server.password) || (typeof (server.passphrase) === 'boolean' && server.passphrase)) {
        ...
        DeployUtils.askUser(util.format('[DEPLOYER] You need to provide a %s for server %s : ', type, server_name), true, (value) => {
          server[type] = value;
          return next(null);
        })
      }
